...
int main(int ac, char *av[])
{
    ...
    log("Preparing websocket server for INDI server...\n");

    using WsServer = Websocket::SocketServer<Websocket::WS>;
    WsServer server;
    server.config.port = 8080;

    auto &indi = server.endpoint["^/indi/?$"];
    indi.on_message = [&server](std::shared_ptr<WsServer::Connection> /*connection*/, std::shared_ptr<WsServer::InMessage> in_message) {
    ...
    indi.on_open = [](std::shared_ptr<WsServer::Connection> connection) {
        std::cout << "Server: Opened connection " << connection.get() << std::endl;
    };

    // See RFC 6455 7.4.1. for status codes
    indi.on_close = [](std::shared_ptr<WsServer::Connection> connection, int status, const std::string & /*reason*/) {
        std::cout << "Server: Closed connection " << connection.get() << " with status code " << status << std::endl;
    };

    indi.on_error = [](std::shared_ptr<WsServer::Connection> connection, const Websocket::error_code &ec) {
        std::cout << "Server: Error in connection " << connection.get() << ". "
            << "Error: " << ec << ", error message: " << ec.message() << std::endl;
    };

    indi.on_handshake = [](std::shared_ptr<WsServer::Connection> /*connection*/, Websocket::CaseInsensitiveMultimap & /*response_header*/) {
        return Websocket::StatusCode::information_switching_protocols; // Upgrade to websocket
    };
    ...
    log("Webscoket server started on port 8080\n");
...
static void usage(void)
{
    ...
    fprintf(stderr, "INDI Library: %s\nProtocol %g.\n", CMAKE_INDI_VERSION_STRING, INDIV);
