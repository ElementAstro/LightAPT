...
from server.script.utils import tool_utils, file_utils
from server.script.utils.process_utils import ProcessInvoker
from server.script.utils.tool_utils import InvalidWebBuildException
from web import server
from web.client import tornado_client_config

parser = argparse.ArgumentParser(description='Launch script-server.')
parser.add_argument('-d', '--config-dir', default=os.path.join('server','config'))
...
def main():
    ...
    migrate.migrate(TEMP_FOLDER, CONFIG_FOLDER, SERVER_CONF_PATH, LOG_FOLDER)
